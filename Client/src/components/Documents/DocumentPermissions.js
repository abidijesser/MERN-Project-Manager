...
const DocumentPermissions = ({ document, visible, onClose, onPermissionsUpdated }) => {
  ...
  const fetchDocumentPermissions = async () => {
    try {
      setLoading(true)
      const response = await axios.get(`/documents/${document.id}/permissions`)
      ...
      if (response.data.success) {
        ...
        setUsers(prevUsers =>
          prevUsers.filter(user =>
            !documentPermissions.some(p => p.user._id === user._id)
          )
        )
      } else {
      ...
      if (response.data.success) {
        ...
        let filteredUsers = response.data.data.filter(
          (user) =>
            ...
            user._id !== document.uploadedBy._id
        )
  ...
  const handleAddPermission = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`/documents/${document.id}/permissions`, {
        userId: selectedUser,
      ...
      <CModalBody>
        ...
        <div className="mb-4">
          ...
          <div className="owner-info d-flex align-items-center p-3 bg-light rounded">
            ...
            <div>
              <div className="fw-bold">{document?.uploadedBy?.name}</div>
              <div className="text-muted small">{document?.uploadedBy?.email}</div>
            </div>
        ...
        <div className="mb-4">
            ...
            <div>
              <CTooltip content={document?.isPublic ? 'Document public' : 'Document privé'}>
                <CBadge color={document?.isPublic ? 'info' : 'secondary'} className="me-2">
                  <CIcon icon={document?.isPublic ? cilPeople : cilLockLocked} className="me-1" size="sm" />
          ...
          {permissions.length === 0 ? (
            <div className="text-center py-4 bg-light rounded">
              <p className="text-muted mb-0">Aucun utilisateur n'a accès à ce document en dehors du propriétaire.</p>
            </div>
          ) : (
                    ...
                    <CTableDataCell>
                      <div className="d-flex">
                        <CFormSelect
                          ...
                          onChange={(e) => handleUpdatePermission(permission.user._id, e.target.value)}
                          disabled={loading}
                        >
