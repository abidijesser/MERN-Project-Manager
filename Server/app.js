...
// Configuration de connexion MongoDB avec options avancées
const mongooseOptions = {
  serverSelectionTimeoutMS: 30000, // Augmenter le délai à 30 secondes (par défaut 10s)
  socketTimeoutMS: 45000, // Augmenter le délai de socket à 45 secondes
...
const connectToDatabase = async () => {
  try {
    // Essayer d'abord de se connecter à MongoDB Atlas (cloud)
    console.log("Tentative de connexion à MongoDB Atlas...");
    await mongoose.connect(process.env.MONGO_URI, mongooseOptions);
    console.log("Cloud Database connected successfully");
    return true;
  } catch (cloudErr) {
    console.error("Erreur de connexion à MongoDB Atlas:", cloudErr.message);

    // Afficher plus d'informations sur l'erreur
    if (cloudErr.name === 'MongooseServerSelectionError') {
      console.error("MongoDB server selection error. Possible causes:");
      console.error("- Network connectivity issues");
      console.error("- MongoDB Atlas service might be down");
      console.error("- IP address might not be whitelisted in MongoDB Atlas");
    }

    // Essayer de se connecter à MongoDB local comme solution de secours
    try {
      console.log("Tentative de connexion à MongoDB local...");
      await mongoose.connect(process.env.MONGO_URI_LOCAL || 'mongodb://localhost:27017/project-manager', {
        ...mongooseOptions,
        serverSelectionTimeoutMS: 5000 // Délai plus court pour la connexion locale
      });
      console.log("Local Database connected successfully");
      return true;
    } catch (localErr) {
      console.error("Erreur de connexion à MongoDB local:", localErr.message);
      console.error("Impossible de se connecter à une base de données MongoDB.");
      console.error("Veuillez vérifier que MongoDB est installé et en cours d'exécution localement,");
      console.error("ou que votre connexion Internet fonctionne pour MongoDB Atlas.");
      return false;
    }
  }
};
...
connectToDatabase().then(connected => {
  if (!connected) {
    console.error("L'application peut ne pas fonctionner correctement sans connexion à la base de données.");
  }
});
  ...
  socket.on("joinRoom", async (room) => {
    ...
    if (room.startsWith("meeting-")) {
      ...
      try {
        ...
        // Get user ID from socket or query parameters
        ...
        if (userId) {
          ...
          if (project) {
            ...
            if (isOwner || isMember) {
              // User is authorized, allow joining the room
              socket.join(room);
              console.log(`User ${userId} authorized to join project room ${room}`);

              // Store project ID and user ID in socket for later use
              socket.projectId = projectId;
              socket.userId = userId;

              // Fetch project messages and send to the user
              try {
              ...
              // User is not authorized
              console.log(`User ${userId} NOT authorized to join project room ${room}`);
              socket.emit("error", {
          ...
          socket.emit("error", {
            message: "User ID is required to join project chat",
          });
        }
      } catch (err) {
        console.error("Error checking project authorization:", err);
        socket.emit("error", {
          message: "Error checking project authorization",
        });
      }
    } else {
      // Not a meeting or project room, allow joining
      socket.join(room);
    }
  });
  ...
  socket.on("sendMessage", async (messageData) => {
    try {
      ...
      if (messageData && messageData.content && messageData.sender) {
        ...
        let messageType = "meeting"; // Default type

        if (messageData.room && messageData.room.startsWith("project-")) {
          messageType = "project";

          // Extract project ID from room
          const projectId = messageData.room.replace("project-", "");
          ...
          if (userId) {
            ...
            if (project) {
              ...
              if (!isOwner && !isMember) {
                // User is not authorized to send messages to this project
                socket.emit("error", {
                  message: "You are not authorized to send messages to this project"
                });
                return;
              }
            } else {
        ...
        // Create a new message with the appropriate type
        ...
        // For project messages, notify other project members
        if (messageType === "project") {
          try {
            const project = await Project.findById(messageData.projectId);
            ...
            if (project && sender) {
              ...
              // Filter unique members and exclude sender
              const uniqueMembers = [...new Set(allMembers.map(id => id.toString()))]
                .filter(id => id !== messageData.sender.toString());
